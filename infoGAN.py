...
class generator(nn.Module):
    ...
    def __init__(self, dataset = 'mnist'):
        ...
        if dataset == 'mnist' or 'fashion-mnist':
            ...
            self.input_dim = self.noise_dim + len_discrete_code + len_continuous_code
            self.output_dim = 3
...
class discriminator(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.fc = nn.Sequential(
            nn.Linear(512 * (self.input_height // 8) * (self.input_width // 8), self.output_dim + self.len_continuous_code + self.len_discrete_code),
            # nn.Sigmoid(),
        )
        utils.initialize_weights(self)
    ...
    def forward(self, input):
        x = self.conv(input)
        x = x.view(-1, 512 * (self.input_height // 8) * (self.input_width // 8))
        x = self.fc(x)
        a = F.sigmoid(x[:, 0])
        b = F.sigmoid(x[:, self.output_dim:self.output_dim + self.len_continuous_code])
        start = self.output_dim + self.len_continuous_code
        c = x[:, self.output_dim + self.len_continuous_code:].clone()
        for n_each_cat in cat:
